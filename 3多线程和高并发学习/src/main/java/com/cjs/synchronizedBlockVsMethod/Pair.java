 ...
 * 
 * 此类的线程安全安全问题：+++++++
 * 	1、x和y的载体在内存中是单例-不用管使用它的类是否是单例
 *  2、++的非原子性,还有对于{@linkplain Pair#checkState()}的约束条件来说存在读写交叉问题:incermentX和incermentY必须同时被一个线程(任务)执行完毕
 *  
 *  他的线程安全问题在{@linkplain PairManager#increment()}得到解决
