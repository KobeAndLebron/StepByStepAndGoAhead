 ...
 * 
 * 	对于成员变量-基本数据类型-来说，主要看载体在heap area是否是单例(多个线程真正操作的实例是否为单例)，如果是单例，那么对此成员变量就必须进行
 * 同步控制在有线程安全的问题存在时。如果不确定是不是为单例，那么在有线程安全的问题存在时，必须进行同步控制。
 * 如果不是单例，则不需要同步；
 ...
 *   	还有一种情况是需要对类的多个方法进行同步在某些特定条件下,即将他们变为原子操作:{@linkplain PairManager#increment()} {@linkplain Pair}
 *    *****************************暂时忽略这种情况
