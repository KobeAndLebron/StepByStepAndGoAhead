...
### Without the quotes(single or double), the shell will interpret *.sh as a glob pattern and expands it to
# any file names matching the glob before passing it to find command.This way, if you had, say, foo.java in
# the current directory, find's actual command(find . -name *.java) line would be:
### find . -name foo.java
